...
function ProductForm() {
  ...
  const productCode = crypto.randomUUID();

  const onSubmit = (data) => {
    if (editItemData) {
      editItem(editItemData.id, data); // Call editItem with the updated data
    } else {
      const id = crypto.randomUUID();
      const newFormData = { ...data, id };
  ...
  return (
    <form
      ...
      className="mx-auto gap-5 md:flex md:w-4/5 md:grid-cols-1 md:flex-col lg:grid lg:w-full lg:grid-cols-3"
    >
      <div>
        <div className="mb-4">
          <label
